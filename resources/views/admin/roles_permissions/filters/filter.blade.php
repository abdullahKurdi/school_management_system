<form action="{{ route('admin.roles_permissions.index') }}" method="get">
    <div class="row">
        <div class="col-6 col-lg-2">
            <div class="form-group">
                <input type="text" name="keyword" value="{{old('keyword', request()->input('keyword'))}}" class="form-control" placeholder="{{__('admin.search')}}">
            </div>
        </div>
        <div class="col-6 col-lg-2">
            <div class="form-group">
                <select name="sort_by" class="form-control">
                    <option value="">{{__('admin.order_by')}}</option>
                    <option value="name" {{old('sort_by',request()->input('sort_by') == 'name'? 'selected':'')}}>Name</option>
                    <option value="created_at" {{old('sort_by',request()->input('sort_by') == 'created_at'? 'selected':'')}}>Created_at</option>
                </select>
            </div>
        </div>
        <div class="col-6 col-lg-2">
            <div class="form-group">
                <select name="order_by" class="form-control">
                    <option value="">{{__('admin.ordering')}}</option>
        ...
        <div class="col-6 col-lg-2">
            <div class="form-group">
                <select name="limit_by" class="form-control">
                    <option value="">{{__('admin.no_records')}}</option>
        ...
        <div class="col-2 col-lg-1">
            <div class="form-group">
                <button type="submit" name="submit" class="btn btn-primary">{{__('admin.search')}}</button>
            </div>
        </div>
    </div>
</form>
